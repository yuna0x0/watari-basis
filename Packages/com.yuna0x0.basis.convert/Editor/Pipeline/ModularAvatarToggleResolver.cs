    ...
    public static class ModularAvatarToggleResolver
    {
            ...
            ConversionSource source)
        {
            ...
            foreach (UnityYamlDocument document in documents)
            {
                ...
                // Relative paths are measured from the relative path root when one is set,
                // else from the component's own object (MergeAnimatorProcessor).
                string prefix = string.Empty;
                if (data.PathMode == PathModeRelative)
                {
                    Transform root = source.Root.transform;
                    Transform from = null;
                    if (data.RelativePathRootFileId != 0L)
                    {
                        resolver.TryResolveTransform(data.RelativePathRootFileId, out from);
                    }

                    if (from == null && !string.IsNullOrEmpty(data.RelativePathRootPath))
                    {
                        from = FindFrom(root, data.RelativePathRootPath);
                    }

                    if (from == null)
                    {
                        resolver.TryResolveTransform(data.OwnerGameObjectFileId, out from);
                    }

                    if (from != null && from.IsChildOf(root))
                    {
                        prefix = PathWithin(root, from);
                    }
                }
